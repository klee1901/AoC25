    ...
    def sumInvalidIds(self):
        ...
        if lengthStartID == lengthEndID:
            ...
            if lengthStartID % 2 == 0:
                ...
                # Add any invalid IDs woth UID between ABC+1 and XYZ-1
                for i in range(firstHalfStartID+1, firstHalfEndID):
                    invalidIdSum += i* 10**(lengthStartID/2) + i
                
                # If gap between ABC and XYZ, check if the invalid ID created by ABC and XYZ are in the range
                ...
                # If ABC and XYZ are coincedent, check if the one possible invalid ID is in range
            ...
            # If range covers more than 2 magnitudes, add all invalid IDs of any intermediate even magnitude
            for length in range(lengthStartID+1, lengthEndID):
                if length % 2 == 0:
                    halfMagnitude = 10**(length/2)
                    for invalidHalf in range(halfMagnitude):
                        invalidIdSum += invalidHalf* halfMagnitude + invalidHalf
            
            # If startID has even magnitude there are possible invalid IDs with same magnitude
            if lengthStartID % 2 == 0:
                
                halfMagnitude = lengthStartID/2

                # Check if invalid ID generated by ABC is in range
                if firstHalfStartID >= secondHalfStartID:
                    invalidIdSum += firstHalfStartID* 10**halfMagnitude + firstHalfStartID
                # Add any invalid ID with generator greater than ABC
                for i in range(firstHalfStartID+1,int(10**halfMagnitude)):
                    invalidIdSum += i* 10**halfMagnitude + i
        
            # If endID has even magnitude there are possible invalid IDs with same magnitude
            if lengthEndID % 2 == 0:
                
                halfMagnitude = lengthEndID/2

                # Check if invalid ID generated by XYZ is in range
                if firstHalfEndID <= secondHalfEndID:
                    invalidIdSum += firstHalfEndID* 10**halfMagnitude + firstHalfEndID
                # Add any invalid ID with generator less than XYZ
                for i in range(int(10**(halfMagnitude-1)),firstHalfEndID):
                    invalidIdSum += i* 10**halfMagnitude + i
